...
modelname = "CNN_预处理_Focal_loss_fixed_平衡采样1_1_"
...
def training(x_train, y_train,maxlen,max_token,embedding_matrix,embedding_dims,batch_size,epochs,logpath,modelpath,modelname):
    ...
    for train, test in kfold.split(x_train, y_train):
        ...
        embedding_layer = Embedding(max_token+1,output_dim=embedding_dims,input_length=maxlen,weights=[embedding_matrix],trainable=True)
        ...
        c1= Convolution1D(64, 1, padding='same')(drp)
        b1 = BatchNormalization()(c1)
        r1 = Activation('relu')(b1)
        c2 = Convolution1D(128, 1, padding='same')(drp)
        b2 = BatchNormalization()(c2)
        r2 = Activation('relu')(b2)
        c3 = Convolution1D(256, 3, padding='same')(drp)
        b3 = BatchNormalization()(c3)
        r3 = Activation('relu')(b3)
        inception = concatenate([r1, r2, r3], axis=-1)
        c4 = Convolution1D(128, 3, padding='same')(inception)
        ...
        model.compile(optimizer='nadam', loss=variant_crossentropy_loss(), metrics=[sensitivity, specificity])#variant_focal_loss(gamma=2., alpha=0.5, rescale = False) variant_crossentropy_loss() categorical_crossentropy_with_cost(cost=200) binary_crossentropy_with_cost(cost=200)'binary_crossentropy'[binary_crossentropy_with_cost(cost=200)] loss=[focal_loss(alpha=.25, gamma=2)] 'categorical_crossentropy'
        model.summary()
        ...
        model.fit(x_train[train], y_train[train],validation_data=(X[test], Y[test]),
            epochs=epochs, batch_size=batch_size,callbacks=callback_lists)#class_weight = {0:1, 1:40},class_weight = {0:1, 1:200},
